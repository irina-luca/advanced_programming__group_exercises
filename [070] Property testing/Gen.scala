...
object Gen {
  ...
  // \lstinline{map} function.
   ...
   def choose (start :Int, stopExclusive :Int) :Gen[Int] = {
    ...
    Gen(State(RNG.nonNegativeLessThan(stopExclusive-start)).map(x=> x+start))
   }
  ...
  // we wrap it in.
   ...
   //Gen(State(RNG.nonNegativeLessThan(2)).map(x=> if(x==0) false else true))
  ...
  // coin with flatMap.
   ...
   def weighted[A](g1: (Gen[A],Double), g2: (Gen[A],Double)): Gen[A] = {
    var g1ch = g1._2*50 //chances of getting g1 times 50% (between 0 and 50)
    var g2ch = g2._2*50 //changes of getting g1 times 50% (between 0 and 50)
    var diff = (100-g1ch-g2ch)/2 //the remainder of 100% - g1ch - g2ch is split equally to both
    Gen.double.flatMap(x=> if(g1ch+diff < x*100) g1._1 else g2._1)
   }
...
case class Prop (run :(TestCases,RNG) => Result) {
  ...
  def || (that :Prop) :Prop = Prop { 
    (n,rng) => run(n,rng) match {
      case Falsified(failure, successes) => that.run( n, rng)
      case x => x
    }
  }
